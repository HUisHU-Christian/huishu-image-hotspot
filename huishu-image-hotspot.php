<?php
/*
Plugin Name: Image Hotspot by DevVN (forked by HUisHU)
Plugin URI: https://levantoan.com/hu-image-hotspot
Description: Image Hotspot help you add hotspot to your images.
Original-Author: Le Van Toan
Author: HUisHU
Version: 1.2.2
...
define('HU_IHOTSPOT_VER', '1.2.2');
define('HU_IHOTSPOT_DEV_MOD', true);
if ( !defined( 'HU_IHOTSPOT_BASENAME' ) )
	define( 'HU_IHOTSPOT_BASENAME', plugin_basename( __FILE__ ) );

define('HU_IHOTSPOT_POINT_DEFAULT',serialize(array(
	'countPoint'	=>	'',
	'content'		=>	'',
	'left'			=>	'',
	'top'			=>	'',
...
define('HU_IHOTSPOT_PINS_DEFAULT', serialize(array(
	'countPoint'	=>	'',
	'imgPoint'		=>	'',
	'top'			=>	'',
	'left'			=>	''
...
add_action( 'add_meta_boxes', 'hu_ihotspot_meta_box' );

function hu_wp_default_editor(){
		return "tinymce";
}

function hu_ihotspot_meta_box_callback( $post ) {
	...
	<?php if (is_array($data_points)):?>
		<?php $stt = 1;foreach ($data_points as $point):?>
			...
			echo hu_ihotspot_get_input_point_default($data_input);?> 
		<?php $stt++;endforeach;?>
	<?php else:?>
		<div style="display: none;"><?php wp_editor('', '_hu_ihotspot_default_content'); ?></div>
	<?php endif;?>	 	 
	</div>
	<?php
}
...
function hu_ihotspot_save_meta_box_data( $post_id ) {
	...
	/*sanitize in hu_ihotspot_convert_array_data*/
	...
	if (is_array($pointdata)){
		$dataPoints = hu_ihotspot_convert_array_data($pointdata);
	}
	$data_post = array(
		'maps_images'		=>	$my_data,
	...
	/*remove_action( 'save_post', 'hu_ihotspot_save_meta_box_data' );
	wp_update_post(array(
		'ID'			=>	$post_id,
		'post_content'	=>	maybe_serialize(wp_unslash($data_post)),
		'post_type'		=>	'points_image'
	));	
	add_action( 'save_post', 'hu_ihotspot_save_meta_box_data' );*/
}
add_action( 'save_post', 'hu_ihotspot_save_meta_box_data' );

function hu_ihotspot_editor_styles(){
	...
	$mce_css = trim( apply_filters( 'hu_ihotspot_mce_css', implode( ',', $mce_css ) ), ' ,' );
...
/*Add admin script*/
function hu_ihotspot_admin_script() {
	global $typenow;
	if ( $typenow == 'points_image' ) {
		...
		/*wp_register_script( 'bootstrap-js', plugin_dir_url( __FILE__ ) . 'admin/js/bootstrap.min.js', array( 'jquery' ), hu_IHOTSPOT_VER, true );
...
add_action( 'admin_enqueue_scripts','hu_ihotspot_admin_script' );

/*Add admin style*/
function hu_ihotspot_admin_styles(){
	global $typenow;
	if ( $typenow == 'points_image' ) {
		wp_enqueue_style( 'bootstrap', plugin_dir_url( __FILE__ ) . 'admin/css/bootstrap.css', array(), HU_IHOTSPOT_VER, 'all' );
		wp_enqueue_style( 'maps_points', plugin_dir_url( __FILE__ ) . 'admin/css/maps_points_style.css', array(), HU_IHOTSPOT_VER, 'all' );
	}
}
add_action( 'admin_print_styles', 'hu_ihotspot_admin_styles' );

/*Add frontend scripts*/
function hu_ihotspot_frontend_scripts() {
	if (HU_IHOTSPOT_DEV_MOD){
		...
		wp_enqueue_style('maps-points',plugin_dir_url( __FILE__ ) . 'frontend/css/maps_points.css',array(), HU_IHOTSPOT_VER,'all');
		wp_enqueue_script( 'maps-points', plugin_dir_url( __FILE__ ) . 'frontend/js/maps_points.js', array('jquery'), HU_IHOTSPOT_VER, true );
	} else {
		wp_enqueue_style('ihotspot',plugin_dir_url( __FILE__ ) . 'frontend/css/ihotspot.min.css',array(), HU_IHOTSPOT_VER,'all');
		wp_enqueue_script( 'ihotspot-js', plugin_dir_url( __FILE__ ) . 'frontend/js/jquery.ihotspot.min.js', array('jquery'), HU_IHOTSPOT_VER, true );		
	}	
}
add_action( 'wp_enqueue_scripts', 'hu_ihotspot_frontend_scripts' );

function hu_ihotspot_get_input_point_default($data = array()){
...
function hu_ihotspot_get_pins_default($datapin = array()){
	if (!is_array($datapin)) $datapin = array();
	$datapin = wp_parse_args($datapin, unserialize(HU_IHOTSPOT_PINS_DEFAULT));
...
//Clone Point
add_action( 'wp_ajax_hu_ihotspot_clone_point', 'hu_ihotspot_clone_point_func' );
function hu_ihotspot_clone_point_func() {
	...
	wp_send_json_success(array(
		'point_pins'	=>	hu_ihotspot_get_pins_default($datapin),
		'point_data'	=>	hu_ihotspot_get_input_point_default($data_input)
	));
	die();
}
...
* */
function hu_ihotspot_convert_array_data($inputArray = array()){
	$aOutput =  array();		
	$firstKey = null;
